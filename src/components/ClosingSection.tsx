    ...
    <div className="max-w-2xl mx-auto text-center">
      <div className="space-y-4 mb-12">
        <motion.p initial={{ opacity: 0, y: 25 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          className="font-display text-2xl md:text-4xl font-light text-primary-foreground/90 italic">You don't need to do more.</motion.p>
        <motion.p initial={{ opacity: 0, y: 25 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          transition={{ duration: 0.8, delay: 0.2, ease: [0.22, 1, 0.36, 1] }}
          className="font-display text-2xl md:text-4xl font-light text-primary-foreground italic">You need to stop.</motion.p>
      </div>
      ...
      <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
        transition={{ duration: 0.6, delay: 0.5 }} className="font-body text-sm text-primary-foreground/50 tracking-wide mb-2">And in that space…</motion.p>
      <motion.p initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}
        transition={{ duration: 1, delay: 0.6, ease: [0.22, 1, 0.36, 1] }}
        className="font-display text-3xl md:text-5xl font-light text-primary-foreground italic mb-14">everything begins to change.</motion.p>
      <motion.a
        href="#book"
